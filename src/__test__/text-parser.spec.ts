...
describe('text-parser', () => {
    ...
    it('must rewrite "js"', () => {
        const name = 'js'
        const result = tp.rewrite(testFile(name), {
            a: 1,
            b: 1,
            c: 2,
            d: 2,
...
function testFile(name: string) {
    return fs
        .readFileSync(path.join(__dirname, 'text-parser', name + '.test'))
        .toString()
}
function codeFile(name: string) {
    return fs
        .readFileSync(path.join(__dirname, 'text-parser', name + '.code'))
        .toString()
}
function saveCodeFile(name: string, data: string) {
    return fs.writeFileSync(
        path.join(__dirname, 'text-parser', name + '.code'),
        data,
    )
}
