...
function testFile(name: string) {
    return fs
        .readFileSync(path.join(__dirname, 'mixed-parser', name + '.test'))
        .toString()
}
function codeFile(name: string) {
    return fs
        .readFileSync(path.join(__dirname, 'mixed-parser', name + '.code'))
        .toString()
}
function saveCodeFile(name: string, data: string) {
    return fs.writeFileSync(
        path.join(__dirname, 'mixed-parser', name + '.code'),
        data,
    )
}
