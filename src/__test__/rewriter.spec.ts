...
describe('rewriter', () => {
    ...
    it('must rewrite plain.html file', () => {
        ...
        expect(readFileSync(outputPath(file)).toString()).toEqual(
            readFileSync(toBePath(file)).toString(),
        )
    })
...
function inputPath(name?: string) {
    return name
        ? path.join(__dirname, 'rewriter', 'input', name)
        : path.join(__dirname, 'rewriter', 'input')
}
...
function outputPath(name?: string) {
    return name
        ? path.join(__dirname, 'rewriter', 'output', name)
        : path.join(__dirname, 'rewriter', 'output')
}
